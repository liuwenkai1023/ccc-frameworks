...
export abstract class SdkAdapterBase implements AdapterInterface {

    abstract openUrl(url: string);
    abstract copyToClipboard(text: string): boolean;
    abstract showAdBanner(isShow: boolean);
    abstract showAdVideo(callback: (result: { code: number; data?: any; }) => void);
